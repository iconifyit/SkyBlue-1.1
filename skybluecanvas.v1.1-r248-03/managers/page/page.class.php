...
class page extends manager
{
    ...
    function InitEditor() 
    {
        ...
        $form['INCLUDE_IN_SITEMAP'] = $this->getIncludeInSiteMap($_OBJ);
    ...
    function getIncludeInSiteMap($_OBJ) {
        global $Core;
        return $Core->YesNoList(
            'include_in_sitemap', 
            Filter::get($_OBJ, 'include_in_sitemap', 1)
        );
    }
    ...
    function GetTypeSelector($selected=null)
    {
        ...
        for ($i=0; $i<count($files); $i++) 
        {
            ...
            $bits = explode('.', $file);
            /**
             * r247 - added code to filter files not named skin.{name}.{ext}
             */
            if (count($bits) != 3) continue;
            if (strcasecmp($bits[0], 'skin') !== 0) continue;
            $type = $bits[1];
